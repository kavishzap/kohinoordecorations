...
export type SocialPlatform = keyof typeof SOCIAL_ICON_PATHS
...
export function buildSocialLinks(company: {
  ...
  const entries: { label: string; platform: SocialPlatform; href: string }[] = [
    { label: "Instagram", platform: "instagram", href: company.instagramLink },
    { label: "Facebook", platform: "facebook", href: company.facebookLink },
    { label: "TikTok", platform: "tiktok", href: company.tiktokLink },
  ]

  return entries
    .filter((e) => e.href)
    .map(({ label, platform, href }) => ({
      label,
      href,
